...
export default function HeroImagesForm({
  ...
  const handleUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (const file of files) {
      if (file.size > 10 * 1024 * 1024) {
        alert(
          `File "${file.name}" is too large. Please upload images smaller than 10MB.`,
        );
        return;
      }
    }
    ...
    try {
      ...
      setError(
        "An unexpected error occurred. Please try smaller files or check your connection.",
      );
    } finally {
  ...
  const handleReorder = async (newOrder: HeroImage[]) => {
    setError("");
    setImages(newOrder);
    const orderedIds = newOrder.map((img, idx) => ({
      id: img.id,
      sortOrder: idx,
    }));
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
            ...
            <div className="md:col-span-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setUploadViewport(e.target.value as HeroViewport)
                }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleViewportChange(
                          img.id,
                          e.target.value as HeroViewport,
                        )
                      }
        ...
        {images.length === 0 && (
          <div className="py-12 text-center bg-stone-50/50 rounded-xl border-2 border-dashed border-stone-200">
            ...
            <p className="text-xs text-stone-400 mt-1">
              The default background will be used on the homepage.
            </p>
          </div>
        )}
      </div>
