  ...
  return (
    <div className="space-y-6">
      <div className="rounded-md border border-amber-100 bg-amber-50/60 p-3 text-xs text-amber-900">
        If the cart contains both clothes and jewelry items, checkout
            ...
            {sortedRules.map((rule) => (
              <tr key={rule.id}>
                ...
                <td className="px-4 py-2 font-medium">
                  ${rule.minAmount.toString()}{" "}
                  {rule.maxAmount ? `- $${rule.maxAmount.toString()}` : "+"}
                </td>
                <td className="px-4 py-2">
                  {Number(rule.price) === 0 ? (
      ...
      <form
        onSubmit={handleAdd}
        className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end bg-white p-4 rounded-md border border-gray-100 shadow-sm"
      >
        <div>
          <label
            htmlFor="shipping-rule-category"
        ...
        <div>
          <label
            htmlFor="shipping-rule-min"
            className="block text-xs font-medium text-gray-500 uppercase mb-1"
          >
            Min Order ($)
          </label>
          <input
            id="shipping-rule-min"
            type="number"
            min="0"
            step="0.01"
            required
        ...
        <div>
          <label
            htmlFor="shipping-rule-max"
            className="block text-xs font-medium text-gray-500 uppercase mb-1"
          >
            Max Order ($)
          </label>
          <input
            id="shipping-rule-max"
            type="number"
            min="0"
            step="0.01"
            value={maxAmount}
        ...
        <div>
          <label
            htmlFor="shipping-rule-price"
            className="block text-xs font-medium text-gray-500 uppercase mb-1"
          >
            Shipping Price ($)
          </label>
          <input
            id="shipping-rule-price"
            type="number"
            min="0"
            step="0.01"
            required
            ...
            className="w-full rounded border border-gray-300 p-2"
            placeholder="10 (0 for Free)"
          />
        </div>
        <button
          type="submit"
          disabled={isLoading}
          className="bg-green-600 text-white rounded p-2 text-sm font-bold hover:bg-green-700 disabled:opacity-50"
        >
          Add Rule
        </button>
      </form>
