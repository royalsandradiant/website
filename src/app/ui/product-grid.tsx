...
export function ProductGrid({
  ...
  products: (Product | ProductWithCategory | null)[];
}) {
  const validProducts = products.filter(
    (p): p is Product | ProductWithCategory => p !== null,
  );
  ...
  const getDisplayPrice = (product: Product) => {
    ...
    const sale =
      product.isOnSale && product.salePrice ? product.salePrice : null;
          ...
          >
            <Link href={`/products/${product.id}`} className="group block">
              ...
              <div className="relative mb-4 aspect-3/4 w-full overflow-hidden bg-[#F0EBE0]">
                ...
                {mainImage ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center">
                    <span className="font-display text-3xl text-foreground/10 italic">
                      RR
                    </span>
                  </div>
                )}
                ...
                <div className="absolute bottom-0 left-0 w-full translate-y-0 lg:translate-y-full bg-white/90 py-2 px-3 backdrop-blur-sm transition-transform duration-300 lg:group-hover:translate-y-0 flex items-center justify-between gap-2">
                  <span className="font-sans text-[9px] font-medium tracking-widest text-foreground uppercase truncate">
                    View Details
                  </span>
                  <button
                    type="button"
              ...
              <div className="text-center">
                <p className="font-sans text-l text-foreground group-hover:text-primary transition-colors">
                  {product.name}
                </p>
