...
function transformProductWithCategory(
  product: any | null,
...
export async function fetchProductsByCategoryPath(slugPath: string) {
  try {
    ...
    const categories = await prisma.category.findMany({
      where: {
        OR: [{ slugPath }, { slugPath: { startsWith: `${slugPath}/` } }],
      },
      select: { id: true },
    });
...
export async function fetchComboSettings() {
  try {
    ...
    if (!prisma.settings) {
      console.warn(
        "prisma.settings is undefined. This may be due to an outdated Prisma client.",
      );
      return {
        comboDiscount2: 10,
        comboDiscount3: 15,
