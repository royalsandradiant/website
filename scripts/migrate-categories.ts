...
const connectionString =
  process.env.POSTGRES_PRISMA_URL_NO_SSL || process.env.DATABASE_URL;
...
const categoryDefinitions: Record<
  string,
  { description: string; subcategories: string[] }
> = {
  dresses: {
    description:
      "Embrace the elegance of traditional Indian dresses, inspired by centuries-old craftsmanship and vibrant cultural motifs. Perfect for weddings, festivals like Diwali, and everyday grace, our collection celebrates the artistry of sarees, lehengas, and more.",
    subcategories: [
      "Sarees",
      "Lehengas",
      "Salwar Kameez",
  ...
  necklaces: {
    description:
      "Discover the royal allure of Indian necklaces, handcrafted with intricate details from Mughal-era designs to temple-inspired motifs. These pieces, often worn during auspicious occasions like weddings, blend gold, gems, and symbolism for timeless beauty.",
    subcategories: [
      "Mangalsutra",
      "Kundan",
      "Polki",
  ...
  bracelets: {
    description:
      "Adorn your wrists with the rhythmic charm of Indian bracelets, drawing from bridal traditions and folk artistry. From sturdy Kadas symbolizing strength to delicate Choodas for celebrations, each piece echoes the vibrancy of Indian heritage.",
    subcategories: [
      "Kada",
      "Chooda",
      "Glass Bangles",
  ...
  earrings: {
    description:
      "Sparkle with the intricate elegance of Indian earrings, influenced by ancient Rajasthani and South Indian styles. Jhumkas swaying to festive dances or subtle studs for daily poise—these designs capture the soul of Indian adornment.",
    subcategories: [
      "Jhumkas",
      "Chandbalis",
      "Kundans",
...
async function main() {
  ...
  for (const [key, def] of Object.entries(categoryDefinitions)) {
    ...
    console.log(
      `Created/Updated parent category: ${parentName} (${parent.id})`,
    );
  ...
  console.log(
    "\nCategory structure created. Total categories:",
    categoryMap.size,
  );
  ...
  for (const product of products) {
    ...
    if (targetCategoryId) {
      ...
      migratedCount++;
      console.log(
        `  Migrated: ${product.name} -> categoryId: ${targetCategoryId}`,
      );
    } else {
      console.log(
        `  WARNING: No category match for product ${product.id} (${legacyCat}/${legacySub})`,
      );
      skippedCount++;
    }
  }
